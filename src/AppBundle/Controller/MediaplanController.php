...
//        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('A3', 'Договор: '.$plan->getContractNumber());
        ...
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('H3', 'Импакт-фактор');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('I3', 'Формат');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('J4', 'I');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('K4', 'II');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('L4', 'III');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('M4', 'IV');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('N4', 'V');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('O4', 'VI');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('P4', 'VII');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('Q4', 'VIII');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('R4', 'IX');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('S4', 'X');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('T4', 'XI');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('U4', 'XII');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('V3', 'Общее кол-во публикаций');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('W3', 'Стоимость размещения, без НДС');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('X3', 'Общий бюджет, без НДС');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('Y3', 'Скидка %');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('Z3', 'Бюджет после скидки, без  НДС');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('AA3', 'НДС 18%');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('AB3', 'Стоимость, включая НДС 18%');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('AC3', 'Агентская комиссия (5%), без НДС');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('AD3', 'Агентская комиссия, включая НДС 18%');
        $phpExcelObject->setActiveSheetIndex(0)->setCellValue('AE3', 'Бюджет после скидки, включая АК, без НДС');
...
//
        ...
        $phpExcelObject->getActiveSheet()->getColumnDimension('AE')->setWidth(15);
...
//          Получаем в0се модули на данный месяц на данный журнал
            ...
            $phpExcelObject->getActiveSheet()->getStyle('X'.$row)->getNumberFormat()->setFormatCode(\PHPExcel_Style_NumberFormat::FORMAT_NUMBER_00);
            $phpExcelObject->getActiveSheet()->getStyle('Y'.$row)->getNumberFormat()->setFormatCode(\PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE_00);
            $phpExcelObject->getActiveSheet()->getStyle('Z'.$row)->getNumberFormat()->setFormatCode(\PHPExcel_Style_NumberFormat::FORMAT_NUMBER_00);
            $phpExcelObject->getActiveSheet()->getStyle('AA'.$row)->getNumberFormat()->setFormatCode(\PHPExcel_Style_NumberFormat::FORMAT_PERCENTAGE_00);
            ...
            $phpExcelObject->getActiveSheet()->getStyle('AE'.$row)->getNumberFormat()->setFormatCode(\PHPExcel_Style_NumberFormat::FORMAT_NUMBER_00);



            $phpExcelObject->setActiveSheetIndex(0)->getStyle('A'.$row.':AE'.$row)->applyFromArray(
                array(
                    'borders' => array(
                        'allborders' => array(
                            'style' => \PHPExcel_Style_Border::BORDER_THIN
            ...
            $phpExcelObject->setActiveSheetIndex(0)->setCellValue('V'.$row, $modulesCount);
            $phpExcelObject->setActiveSheetIndex(0)->setCellValue('W'.$row, '='.$price.'/1.18');
            $phpExcelObject->setActiveSheetIndex(0)->setCellValue('X'.$row, '=U'.$row.'*V'.$row);
            $phpExcelObject->setActiveSheetIndex(0)->setCellValue('Y'.$row, ($good->getSale()/100));
            $phpExcelObject->setActiveSheetIndex(0)->setCellValue('Z'.$row, '=W'.$row.'*(1-X'.$row.')');
            $phpExcelObject->setActiveSheetIndex(0)->setCellValue('AA'.$row, '0.18');
            $phpExcelObject->setActiveSheetIndex(0)->setCellValue('AB'.$row, '=Y'.$row.'+(Y'.$row.'*Z'.$row.')');
            $phpExcelObject->setActiveSheetIndex(0)->setCellValue('AC'.$row, '=Y'.$row.'*'.($plan->getCommission()/100));
            $phpExcelObject->setActiveSheetIndex(0)->setCellValue('AD'.$row, '=AB'.$row.'*1.18');
            $phpExcelObject->setActiveSheetIndex(0)->setCellValue('AE'.$row, '=AA'.$row.'+AC'.$row);
            $row ++;
        }

            $phpExcelObject->getActiveSheet()->mergeCells('C'.$row.':AE'.$row);
            ...
            $phpExcelObject->setActiveSheetIndex(0)->setCellValue('AE'.$row, '=SUM(AE5:AE'.($row-1).')');
            $phpExcelObject->getActiveSheet()->getStyle('AE'.$row)->getNumberFormat()->setFormatCode(\PHPExcel_Style_NumberFormat::FORMAT_NUMBER_00);
            $phpExcelObject->getActiveSheet()->getStyle('A'.$row.':AE'.$row)->getFont()->setBold(true);
